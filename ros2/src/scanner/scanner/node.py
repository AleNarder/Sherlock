...
class ScannerNode(Node, StateMachine):
    ...
    def __init__(self):
        ...
        # Services
        self.create_service(
            Trigger, "/scanning/recorder/start_record", self._on_start_record_cb
        )
        self.create_service(
            Trigger, "/scanning/recorder/stop_record", self._on_stop_record_cb
        )
    ...
    def _on_depth_to_color_cb(self, msg: Extrinsics):
        ...
        if self.current_state == self.UNITIALIZED:
            ...
            if (
                self.hand_eye_tf_are_loaded()
                and self.intrinsics_are_loaded()
                and self.depth_camera_info_are_loaded()
            ):
                self.initialize()
    ...
    def _on_intrinsics_coeffs_cb(self, msg: String):
        if self.current_state == self.UNITIALIZED:
            ...
            if (
                self.hand_eye_tf_are_loaded()
                and self.depth_camera_info_are_loaded()
                and self.depth_2_color_is_loaded()
            ):
                self.initialize()

    def _on_hand_eye_cb(self, msg: Extrinsics):
        if self.current_state == self.UNITIALIZED:
            self.hand_eye_h = homogenous_from_rt(
                np.array(msg.rotation).reshape(3, 3), msg.translation
            )
            if (
                self.depth_camera_info_are_loaded()
                and self.intrinsics_are_loaded()
                and self.depth_2_color_is_loaded()
            ):
                self.initialize()
    ...
    def _on_depth_info_cb(self, msg: CameraInfo):
        if self.current_state == self.UNITIALIZED:
            ...
            if (
                self.intrinsics_are_loaded()
                and self.hand_eye_tf_are_loaded()
                and self.depth_2_color_is_loaded()
            ):
                self.initialize()
    ...
    def generate_points(self, depth_msg: Image, color_msg: Image, base2eye_h: np.array):
        ...
        valid = (
            (u >= 0)
            & (u < color_image.shape[1])
            & (v >= 0)
            & (v < color_image.shape[0])
        )
        points = points[valid]
    ...
    def _publish_pc(self):
        ...
        pc2_msg.point_step = (
            16  # Number of bytes for each point (3*4 for xyz + 4 for rgb)
        )
