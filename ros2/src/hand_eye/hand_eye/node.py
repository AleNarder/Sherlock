...
class HandEyeNode(StateMachine, Node):
    ...
    initialize = (
        UNITIALIZED.to(VALIDATED, cond=["is_validated"])
        | UNITIALIZED.to(
            PENDING_VALIDATION,
            cond=[
        ...
        | UNITIALIZED.to(INITIALIZED, cond=["is_initialized"])
    )
    ...
    def __init__(
        ...
        if os.path.exists(os.path.join(data_folder, "intrinsics", "mtx.npy")):
            self.intrinsics["mtx"] = np.load(
                os.path.join(data_folder, "intrinsics", "mtx.npy")
            )
            self.get_logger().info("mtx loaded!")

        if os.path.exists(os.path.join(data_folder, "intrinsics", "dist.npy")):
            self.intrinsics["dist"] = np.load(
                os.path.join(data_folder, "intrinsics", "dist.npy")
            )
            self.get_logger().info("distortion loaded!")

        self.eye2hand_hs = []
        if os.path.exists(
            os.path.join(self.data_folder, "hand_eye", "processing", "eye2hand_hs.npy")
        ):
            self.eye2hand_hs = np.load(
                os.path.join(data_folder, "hand_eye", "processing", "eye2hand_hs.npy")
            )
    ...
    def is_initialized(self):
        return (
            self.intrinsics["mtx"] is not None and self.intrinsics["dist"] is not None
        )
    ...
    def on_enter_PROCESSING(self):
        if os.path.exists(os.path.join(self.data_folder, "hand_eye", "processing")):
            shutil.rmtree(
                os.path.join(self.data_folder, "hand_eye", "processing"),
                ignore_errors=True,
            )
    ...
    def before_start_recording(self):
        ...
        self.drifts = []
        os.makedirs(
            os.path.join(self.data_folder, "hand_eye", "processing"), exist_ok=True
        )
    ...
    def after_stop_reprojecting(self):
        os.makedirs(
            os.path.join(self.data_folder, "hand_eye", "reprojecting"), exist_ok=True
        )
        # Save data for debugging and further processing
        np.save(
            os.path.join(self.data_folder, "hand_eye", "reprojecting", "frames.npy"),
            self.frames,
        )
        np.save(
            os.path.join(
                self.data_folder, "hand_eye", "reprojecting", "target2base_hss.npy"
            ),
            self.target2base_hss,
        )
    ...
    def on_enter_PENDING_VALIDATION(self):
        for h, (method_name, _) in zip(self.eye2hand_hs, CALIB_HAND_EYE_METHODS):
            ...
            static_transform_stamped.child_frame_id = (
                self.eye_frame_id + "_" + method_name
            )
    ...
    def process_recording(self, msg: Image):
        ...
        pose_stamp = (
            hand2base_tf.header.stamp.sec + hand2base_tf.header.stamp.nanosec * 1e-9
        )
    ...
    def process_reprojecting(self, msg: Image):
        ...
        if ret:
            ...
            for it, (method_name, method) in enumerate(CALIB_HAND_EYE_METHODS):
                ...
                ret, eye2base_tf, eye2base_h = self._get_transform(
                    self.base_frame_id, eye_frame_id, stamp=timestamp
                )
                if not ret:
                    continue
    ...
    def _get_hand2base(self, stamp=None):
        ...
        if not ret:
            self.get_logger().info(
                "Unable to recover consistent pose for current image"
            )
            return False, None, None
        ...
        while len(hand2base_hs) > min_samples and curr_it < max_iterations:
            ...
            try:
                ...
                for i, mse in enumerate(method_mses):
                    self.get_logger().info(
                        CALIB_HAND_EYE_METHODS[i][0] + ": " + str(mse) + "mm^2"
                    )
                self.get_logger().info("=" * 10)
                ...
                os.mkdir(
                    os.path.join(
                        self.data_folder, "hand_eye", "processing", f"it_{curr_it}"
                    )
                )
                np.save(
                    os.path.join(
