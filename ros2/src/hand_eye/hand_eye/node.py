...
from hand_eye.utils import compute_hand_eye, homogenous_matrix_from_rt, find_chessboard_corners, P_cam2ros
...
class HandEyeNode (StateMachine, Node):
    ...
    def _on_image_cb(self, msg: Image):
        if self.current_state == self.RECORDING:
            ...
            frame_is_sharp = is_sharp(cv_frame, 250)
            ...
            pose, pose_tf = self.get_homogeneous_matrix(timestamp)
                
            if pose is None:
                self.get_logger().info("Unable to recover consistent pose for current image")
                return
            
            self.get_logger().info(f"time delta is: 0.{pose_tf.header.stamp.nanosec - timestamp.nanosec}")
            ...
            if err > 0.1:
                self.get_logger().info("Chessboard proj err too high")
                return
            else:
            ...
            t.header.frame_id = self.eye_frame_id
    ...
    def _on_tf (self, msg: TFMessage):
        if self.current_state != self.PROCESSING:
    ...
    def get_homogeneous_matrix(self, stamp):
        try:
            gripper2base = self.tf_buffer.lookup_transform(self.base_frame_id, self.hand_frame_id_, stamp)
            
            translation = [
                gripper2base.transform.translation.x,
                gripper2base.transform.translation.y,
                gripper2base.transform.translation.z
            ]
            
            rotation = [
                gripper2base.transform.rotation.w,
                gripper2base.transform.rotation.x,
                gripper2base.transform.rotation.y,
                gripper2base.transform.rotation.z
            ]
            
            R = transforms3d.quaternions.quat2mat(rotation)
            t = np.array(translation)

            h = homogenous_matrix_from_rt(R, t)
            
            self.get_logger().info(f"Transform lookup successful!")
            
            return h, gripper2base
        except Exception as e:
    ...
    def _get_hand_eye_tf (self, timestamp):    
        ...
        rot   = self.hand_eye_h[:3, :3]
        trans = self.hand_eye_h[:3, 3] 
        
        quaternion  = transforms3d.quaternions.mat2quat(rot)
        translation = trans.reshape(3)
    ...
    def _process(self):
        ...
        he, hs_gripper2base, hs_target2cam = compute_hand_eye(
            self.frames, 
            self.poses, 
            self.intrinsics, 
            self.get_logger()
