...
class IntrinsicsNode(StateMachine, Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            Image, "/camera/color/image_raw", self._on_image_cb, 10
        )

        # Services
        self.create_service(
            Trigger, "/intrinsics/start_recording", self._start_recording_cb
        ),
        self.create_service(
            Trigger, "/intrinsics/stop_recording", self._stop_recording_cb
        )
        ...
        if os.path.exists(
            os.path.join("/", "calibration", "data", "intrinsics", "mtx.npy")
        ):
        ...
        if os.path.exists(
            os.path.join("/", "calibration", "data", "intrinsics", "dist.npy")
        ):
    ...
    def _on_image_cb(self, msg: Image):
        if self.current_state == self.RECORDING:
            ...
            if is_sharp(cv_frame, 280):
                ...
                corner_frame = cv2.drawChessboardCorners(
                    cv_frame, (9, 6), corners, True
                )
    ...
    def _publish(self):
        ...
        data = {}
        data["mtx"] = (
            self.intrinsics["mtx"].tolist()
            if self.intrinsics["mtx"] is not None
            else None
        )
        data["dist"] = (
            self.intrinsics["dist"].tolist()
    ...
    def _process(self):
        rep_frames = [
            self.frames[idx] for idx in get_n_representative_frames(self.frames)
        ]
        mtx, dist, rvecs, tvecs, objectpoints, imgpoints = compute_intrinsics(
            rep_frames, (9, 6), (640, 480)
        )
        err, _ = compute_reprojection_error(
            objectpoints, imgpoints, rvecs, tvecs, mtx, dist
        )
    ...
    def _on_save_intrinsics(self):
        ...
        np.save(
            os.path.join("/", "calibration", "data", "intrinsics", "err.npy"),
            np.array(self.intrinsics["err"]),
        )
        np.save(
            os.path.join("/", "calibration", "data", "intrinsics", "frames.npy"),
            self.frames,
        )
