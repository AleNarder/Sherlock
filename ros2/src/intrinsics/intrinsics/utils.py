...
def get_n_representative_frames(
    frames: list[cv2_t.MatLike], num_frames=75
) -> list[int]:
    ...
    if len(frames) < num_frames:
        ...
        reduced_frames = [
            cv2.resize(frames[i], None, fx=0.6, fy=0.6, interpolation=cv2.INTER_LINEAR)
            for i in range(0, len(skip_frames), 1)
        ]
        gray_flattened_frames = []
        for frame in reduced_frames:
            gray_flattened_frames.append(
        ...
        print(
            f"Reduced frames from {len(gray_flattened_frames[0])} to {len(pca_reduced_frames[0])} dimensions keeping {expected_variance * 100} of variance"
        )

        # Perform kmeans clustering on pca reduced frames to obtain representative frames
        kmeans = KMeans(n_clusters=num_frames, random_state=0, n_init="auto").fit(
            pca_reduced_frames
        )
        ...
        for cluster_idx, cluster_center in enumerate(kmeans.cluster_centers_):
            ...
            nearest_idx = np.argmin(
                [
                    euclidean_distance(pca_reduced_frames[idx], cluster_center)
                    for idx in cluster_items_idxs
                ]
            )

            representative_frames_idxs.append(cluster_items_idxs[nearest_idx] * 2)
        print(
            f"Selected {len(representative_frames_idxs)} representative frames out of {len(reduced_frames)} frames"
        )
